...
type TextoCobrancaType = {
    selecionada: number,
    sair: () => void,
    set: (id: number | null) => void
}

export default function TextoCobrancaConfig({ selecionada, sair, set }: TextoCobrancaType) {
    ...
    const mensagens: Mensagem[] = [
        ...
        { id: 2, msg: 'Olá, **Nome**. Consta em nosso sistema um em aberto de **Valor** referente a **Quantidade** nota(s). Poderia nos encaminhar o comprovante?' },
    ...
    const [mensagemSelecionada, setMensagemSelecionada] = useState<number | null>(selecionada)
    ...
    return (
        <Cortina onClick={sair}>
            <Container tamanho="m">
                ...
                <div className="bg-white p-2 w-full grid md:grid-cols-2 gap-4 rounded-lg mt-2 overflow-scroll max-h-90 ">
                        ...
                        return (
                            <div key={m.id} className="flex flex-col items-center m-auto w-full ">
                                <label
                                    ...
                                    <span className="whitespace-pre-line text-white text-start px-2 text-sm md:text-base" >
                ...
                <button
                    onClick={() => {
                        set(mensagemSelecionada)
                        sair()
                    }}
                    className="bg-[#005c4b] cursor-pointer text-white px-4 py-2 rounded-lg mt-4 shadow-md hover:bg-[#004d40] transition-colors">
                    Salvar
                </button>
